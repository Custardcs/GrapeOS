...
fn efi_main(image_handle: Handle, mut system_table: SystemTable<Boot>) -> Status {
    ...
    match load_kernel(image_handle, &mut system_table, kernel_path_cstr) {
        Ok(kernel_entry) => {
            ...
            // Add detailed logging right before jumping to kernel
            info!(
                "Preparing to jump to kernel entry point: 0x{:x}",
                kernel_entry
            );
            info!("BootInfo address: {:#x}", boot_params_addr as usize);
            info!("BootInfo contents:");
            info!("  framebuffer_addr: 0x{:x}", boot_info.framebuffer_addr);
            info!("  framebuffer_width: {}", boot_info.framebuffer_width);
            info!("  framebuffer_height: {}", boot_info.framebuffer_height);
            info!("  framebuffer_stride: {}", boot_info.framebuffer_stride);
            info!("  memory_map_addr: 0x{:x}", boot_info.memory_map_addr);
            info!("  memory_map_size: {}", boot_info.memory_map_size);
            info!(
                "  memory_map_entry_size: {}",
                boot_info.memory_map_entry_size
            );

            // Stall for a few seconds to observe the debug output and test pattern
            system_table.boot_services().stall(5_000_000); // 5 seconds

            info!("Testing framebuffer before jumping to kernel...");
            unsafe {
                if boot_info.framebuffer_addr != 0 {
                    // Create a slice for the framebuffer (BGR format)
                    let fb_slice = core::slice::from_raw_parts_mut(
                        boot_info.framebuffer_addr as *mut u32,
                        boot_info.framebuffer_width * boot_info.framebuffer_height,
                    );

                    // Draw a red rectangle at the top of the screen (red in BGR format is 0x000000FF)
                    for y in 0..50 {
                        for x in 0..boot_info.framebuffer_width {
                            let idx = y * boot_info.framebuffer_stride + x;
                            if idx < fb_slice.len() {
                                fb_slice[idx] = 0x000000FF; // Red in BGR
                            }
                        }
                    }

                    info!("Framebuffer test complete - red rectangle should be visible");
                } else {
                    info!("ERROR: Framebuffer address is zero!");
                }
            }
...
fn load_kernel(
    ...
    for i in 0..ph_count {
        ...
        // Important: Allocate memory at the EXACT virtual address specified in the ELF
        // NOT at any arbitrary address
        info!(
            "Allocating {} pages at exact address 0x{:x}",
            pages, ph.p_vaddr
        );
        let segment_addr = boot_services
            .allocate_pages(
                AllocateType::Address(ph.p_vaddr), // Use Address type to place at exact location
                MemoryType::LOADER_DATA,
                pages as usize,
            )
            .map_err(|err| {
                info!("Failed to allocate memory at 0x{:x}: {:?}", ph.p_vaddr, err);
                Status::OUT_OF_RESOURCES
            })?;

        // Verify the allocated address is correct
        if segment_addr != ph.p_vaddr {
            info!(
                "ERROR: Segment allocated at wrong address: requested=0x{:x}, got=0x{:x}",
                ph.p_vaddr, segment_addr
            );
        ...
        info!(
            "Copying segment data: src=0x{:x}, dst=0x{:x}, size={}",
            src, dst, size
        );
        ...
        unsafe {
            ...
            if ph.p_memsz > ph.p_filesz {
                let zero_size = (ph.p_memsz - ph.p_filesz) as usize;
                info!(
                    "Zeroing additional memory: addr=0x{:x}, size={}",
                    dst + ph.p_filesz,
                    zero_size
                );
                core::ptr::write_bytes((dst + ph.p_filesz) as *mut u8, 0, zero_size);
            }
        }
    }
